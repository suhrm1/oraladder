...
def player(profile_id):
    ...
    group_stage_completion = min(
        1, max(0, (date.today() - cfg["START_TIME"]) / (group_stage_end_time - cfg["START_TIME"]))
    )
    ...
    if player_info["status"] == "SF":
        ...
        status = (
            f"⚠️ Late by {matchup_expected_done-matchup_done_count} matchup(s): "
